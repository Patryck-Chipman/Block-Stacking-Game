...
public class ClassicProgressBar : MonoBehaviour {
	...
	public void Awake() {
		Image segmentFillImage = m_Image;
        ...
        m_Image.GetComponent<Image>().SetNativeSize();
		...
		for (int i = 0; i < m_NumberOfSegments; i++) {
			GameObject currentSegment = m_Image.gameObject;//Instantiate(m_Image.gameObject, transform.position, Quaternion.identity, transform);
			currentSegment.SetActive(true);

			Image segmentImage = currentSegment.GetComponent<Image>();
			segmentImage.fillAmount = m_SizeOfSegment;
			//segmentImage.GetComponent<Image>().SetNativeSize();
			...
			segmentFillImage = segmentImage.transform.GetChild (0).GetComponent<Image> ();
			segmentFillImage.GetComponent<Image>().SetNativeSize();
